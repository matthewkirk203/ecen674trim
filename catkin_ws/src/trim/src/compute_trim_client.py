...
if __name__ == "__main__":
    if len(sys.argv) == 4:
        ...
        print usage()
        sys.exit(1)
    print "Requesting trim with Va: %s, gamma: %s, R: %s"%(Va, gamma, R)
    x_trim, u_trim = compute_trim_client(Va, gamma, R)
    print('\n\n x trim')
    print('pn',x_trim[0])
    print('pe',x_trim[1])
    print('pd',x_trim[2])
